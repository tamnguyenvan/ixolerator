...
from meghnad.repo.obj_det.yolo_zsd.models.experimental import attempt_load
from meghnad.repo.obj_det.yolo_zsd.models.yolo import Model
from meghnad.repo.obj_det.yolo_zsd.utils.autoanchor import check_anchors
from meghnad.repo.obj_det.yolo_zsd.utils.datasets import create_dataloader
from utils.general import labels_to_class_weights, increment_path, labels_to_image_weights, init_seeds, \
    fitness, strip_optimizer, get_latest_run, check_dataset, check_file, check_git_status, check_img_size, \
    check_requirements, print_mutation, set_logging, one_cycle, colorstr
from meghnad.repo.obj_det.yolo_zsd.utils.google_utils import attempt_download
from meghnad.repo.obj_det.yolo_zsd.utils.loss import ComputeLoss, ComputeZSDLoss
from meghnad.repo.obj_det.yolo_zsd.utils.plots import plot_images, plot_labels, plot_results, plot_evolution
from meghnad.repo.obj_det.yolo_zsd.utils.torch_utils import ModelEMA, select_device, intersect_dicts, torch_distributed_zero_first, is_parallel
from meghnad.repo.obj_det.yolo_zsd.utils.wandb_logging.wandb_utils import WandbLogger, check_wandb_resume
...
def train(opt: object) -> str:  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        model = Model(opt.cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get(
            'anchors'), hyp=hyp).to(device)  # create
    ...
    if opt.zsd and opt.initial_bg_path:
        model.model[-1].bg = nn.Parameter(torch.load(
            opt.initial_bg_path, map_location=device), requires_grad=True)
    # Freeze
    ...
    if opt.zsd:
        if hyp['sim_func'] == 0:
            optimizer.add_param_group(
                {'params': model.model[-1].sim_func.temp, 'lr': hyp['lr0'] * hyp['learnable_temp_scale']})
        elif hyp['sim_func'] == 1:
            optimizer.add_param_group(
                {'params': model.model[-1].sim_func.cosine_scalar.shift, 'lr': hyp['lr0'] * hyp['learnable_shift_scale']})
            optimizer.add_param_group(
                {'params': model.model[-1].sim_func.cosine_scalar.contrast, 'lr': hyp['lr0'] * hyp['learnable_contrast_scale']})
    ...
    if opt.linear_lr:
        def lf(x): return (1 - x / (intended_epochs - 1)) * \
    ...
    if pretrained and (not opt.skip_resume):
        ...
        if ema and ckpt.get('ema'):
            ema.ema.load_state_dict(
                ckpt['ema'].float().state_dict(), strict=False)
    ...
    # P, R, mAP@.5, mAP@.5-.95, val_loss(box, obj, cls, self_losses(if zsd))
    results = (0, ) * (11 if opt.zsd else 7)
    scheduler.last_epoch = start_epoch - 1  # do not move
    scaler = amp.GradScaler(enabled=cuda)
    compute_loss = ComputeZSDLoss(
        model) if opt.zsd else ComputeLoss(model)  # init loss class
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if opt.zsd:
            logger.info(('\n' + '%10s' * 11) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls',
                        'img_cls', 'text_cls', 'simg_cls', 'stext_cls', 'total', 'labels'))
        else:
            logger.info(('\n' + '%10s' * 8) % ('Epoch', 'gpu_mem',
            ...
            if rank in [-1, 0]:
                ...
                if plots and ni < 3:
                    ...
                    Thread(target=plot_images, args=(
                        imgs, targets[:, :6], paths, f, names), daemon=True).start()
                    if tb_writer:
                        tb_writer.add_graph(torch.jit.trace(
                            model, imgs, strict=False), [])  # add model graph
...
def _build_opt(opt: object) -> Tuple:
    ...
    if opt.hyp and isinstance(opt.hyp, str):
        opt.hyp = get_meghnad_repo_dir() / 'yolo_zsd' / opt.hyp
...
#                         help='evaluate single class but train multi')
...
#     parser.add_argument('--annot-folder', type=str, default='labels',
#                         help='optional argument to change the path to folders relative to images')
#     parser.add_argument('--canonical-epochs', type=int, default=200,
#                         help='number of intended epochs during hyp evolution')
#     parser.add_argument('--verbose', action='store_true')
#     parser.add_argument('--iou-thres', type=float,
#                         default=0.7, help='iou value for testing')
#     parser.add_argument('--obj-conf-thresh', type=float,
#                         default=0.1, help='obj conf thresh for zsd detections')
#     parser.add_argument('--max-det', type=int, default=300,
#                         help='maximum detections for inference')
#     parser.add_argument('--zsd', action='store_true')
#     parser.add_argument('--skip-resume', action='store_true')
#     parser.add_argument('--eval-splits', default=[], type=str, nargs='+',
#                         help='keys for data class splits, eg: "seen_classes"')
...
#     parser.add_argument('--visualization-demo',
#                         default=False, action='store_true')
#     parser.add_argument('--no-zsd-post', default=False, action='store_true',
#                         help='performs regular postprocessing when performing zsd')
#     parser.add_argument('--nms-then-zsd', default=False, action='store_true',
#                         help='performs zsd post-processing after nms')
    ...
    #                 hyp[k] = float(x[i + (11 if opt.zsd else 7)]
    #                                * v[i])  # mutate
