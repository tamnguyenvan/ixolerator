# Dataset trn_utils and dataloaders
...
from meghnad.repo.obj_det.yolov7.utils.general import check_requirements, xyxy2xywh, xywh2xyxy, xywhn2xyxy, xyn2xy, segment2box, segments2boxes, \
    resample_segments, clean_str
from meghnad.repo.obj_det.yolov7.utils.torch_utils import torch_distributed_zero_first
...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    # Use torch.trn_utils.data.DataLoader() if dataset.properties will update during training else InfiniteDataLoader()
    ...
    if probability and n:
        ...
        for j in random.sample(range(n), k=round(probability * n)):
            ...
            box = l[1].astype(int).clip(0, w - 1), l[2].astype(int).clip(
                0, h - 1), l[3].astype(int).clip(0, w - 1), l[4].astype(int).clip(0, h - 1)
...
# from trn_utils.datasets import *; extract_boxes('../coco128')
...
def autosplit(path='../coco', weights=(0.9, 0.1, 0.0), annotated_only=False):
    ...
    Usage: from trn_utils.datasets import *; autosplit('../coco')
    Arguments
